...
[`docs/REMOTE_ACCESS.md`](docs/REMOTE_ACCESS.md)。交互式 `tokdash setup` 可在用户选择启用后，
...
- **服务暴露**：Tokdash 默认绑定 `127.0.0.1`。Tailscale Serve 提供私有只读访问，SSH 转发提供经过认证的写入访问；`--bind 0.0.0.0` 会在所有接口上显式暴露未经认证的只读访问。详见[远程访问指南](docs/REMOTE_ACCESS.md)。
